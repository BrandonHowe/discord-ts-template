...
const code = `function blah <T extends string>(val: T): T {}`;
...
const displayTypeString = (node: ts.Node): string => {
    if (ts.isLiteralTypeNode(node)) {
        ...
        if (
            ...
            Guard<typeof literal, ts.BooleanLiteral>(
                literal,
            ...
            Guard<typeof literal, ts.NumericLiteral>(
                literal,
        ...
        const types = node.parameters.map(l =>
            l.type ? displayTypeString(l.type) : ts.SyntaxKind[node.kind]
        );
        ...
        const names = node.members.map(l =>
            ts.isIndexSignatureDeclaration(l)
                ? l.name || displayTypeString(l)
                : l.name
                ? displayTypeString(l.name)
                : "err"
        );
        const types = node.members.map(l =>
            // TODO: Get rid of this ts-ignore
            // @ts-ignore
            l.type ? displayTypeString(l.type) : "err"
        );
        const zipped = names.map((l, idx) => ({ name: l, type: types[idx] }));
        return `{ ${zipped.reduce(
            (acc, cur) =>
                (acc += `${acc === "" ? "" : ", "}${cur.name}: ${cur.type}`),
            ""
        )} }`;
    } else if (ts.isFunctionTypeNode(node)) {
        const paramTypes = node.parameters.map(l => ({
            name: displayTypeString(l.name),
            type: l.type ? displayTypeString(l.type) : "err"
        }));
        const returnType = displayTypeString(node.type);
        return `(${paramTypes.reduce(
            (acc, cur) => (acc += `${acc === "" ? "" : ", "}${cur.type}`),
            ""
        )}) => ${returnType}`;
    } else if (ts.isTypeReferenceNode(node)) {
...
function printRecursiveFrom(node: ts.Node, debug: boolean) {
    const indentation = "-";
        ...
        const generics =
            ...
            node.typeParameters.map(
                l =>
                    `${l.name.text}${
                        l.constraint
                            ? ` extends ${displayTypeString(l.constraint)}`
                            : ""
                    }`
            );
        const genericsString =
            generics &&
            `<${generics.reduce((acc, cur) => (acc += ` ${cur}`))}>`;
    ...
    node.forEachChild(child => printRecursiveFrom(child, debug));
}

const parseString = (str: string, debug = false) => {
    const sourceFile = ts.createSourceFile(
        "parsing.ts",
        str,
        ts.ScriptTarget.Latest
    );
    printRecursiveFrom(<ts.Node>(<unknown>sourceFile), debug);
};

parseString(code, false);
